...
- Lists visible Dice search postings in the popup and opens selected postings in new tabs.
...
| Dice | Job-detail pages, using `JobPosting` JSON-LD when available. Search result pages can list visible postings and open selected detail pages. |
...
The extension reads the job page you are viewing. On Dice search results, it can open selected visible postings from the current page in new tabs. It does not crawl job boards or submit applications.

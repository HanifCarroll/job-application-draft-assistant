...
def build_draft_prompt(request: DraftRequest, selection: ContextSelection, profile: str) -> str:
    packet = {
        "profile": profile,
        "upwork_project": request.project.model_dump(),
    ...
    return f"""You write Upwork proposal drafts for Hanif Carroll.
...
Rules:
- Keep the proposal concise by default: 120-180 words unless the project clearly needs more.
- Open on the client's problem, not a biography.
...
- Avoid "Dear hiring manager", "I am excited to apply", generic flattery, and desperate sales language.
- Every proposal claim must appear in `claims[]` with `caused_by` refs from source_evidence.
...
Humanize this Upwork proposal draft.

Constraints:
- Return JSON matching the same schema.
- Rewrite only the proposal text and wording-level rationale if needed.
